(* The Kaggriculture action vocabulary: typed emit constructors.
   ...
   These carry the per-game emit signatures — the same arities and operand
   kinds as WORKER_EMITS and MARKET_EMITS in submission/actions.py — so that a
   family author cannot build a malformed emit at all. The generic
   Family.emit stays reachable, which is why the Python loader's emit checks
