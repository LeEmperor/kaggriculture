(* monocrop-reorder-v1 as a typed OCaml value.
   ...
   The authoring source for experiments/policies/monocrop_reorder/family.json: the emitter
   serializes this value, and the emitted JSON is what the Python interpreter loads and
   what the golden vectors prove correct. A change to the family belongs here first; the
...
let liquidation_start_day = Expr.Param.int "liquidation_start_day" ~min:0 ~max:29

(* Registers: the per-episode state. Decision registers are the ones a guard reads; every
   backend must agree on them. Telemetry may diverge harmlessly. *)
let mode =
  Expr.Reg.enum
    "mode"
...
let mode_entered_step = Expr.Reg.int "mode_entered_step" ~init:0 ~cls:Expr.Telemetry
...
let last_money_delta = Expr.Reg.int "last_money_delta" ~init:0 ~cls:Expr.Telemetry
...
let requested_sell_units = Expr.Reg.int "requested_sell_units" ~init:0 ~cls:Expr.Telemetry
let w = Family.write
...
let family =
  Family.create
    ...
    ~observe:
      ...
      ; w
          mode_entered_step
          (if_ (next mode <>: state mode) (obs V.step) (state mode_entered_step))
      ]
    ~market_rules:
      [ Family.rule
          ~name:"sell_stock"
    ...
    ~farmer_cascade:
      ...
      ; Family.rule
          ...
          ~when_:
            (and_
               [ obs V.tile_is_plant
               ; obs V.day -: obs V.tile_planted_day >=: param harvest_min_age_days
