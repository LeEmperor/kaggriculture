(* CPython-exact random.Random: MT19937 plus the exact draw methods upstream uses.
   ...
   [random.Random ((seed * 1_000_003) lxor day)] — and draws only [rng.random ()] (weed
   spawns) and [rng.choice] (shop unlocks). Exact differential replay therefore needs
   CPython's integer seeding (init_by_array over the 32-bit little-endian words of
   [abs n]), its [random ()] (the 53-bit genrand_res53 construction), and its
   [choice]/[_randbelow] (rejection sampling over [getrandbits]).

   Everything is masked to 32 bits explicitly; OCaml's 63-bit native int holds every
   intermediate product (largest is 32-bit x 32-bit = 62 bits). test/python_random_test.ml
   proves the outputs against draws recorded from CPython itself. *)
...
let init_by_array key =
  ...
  for _ = 1 to max n key_length do
    mt.(!i)
    <- (mt.(!i) lxor (mt.(!i - 1) lxor (mt.(!i - 1) lsr 30) * 1664525)) + key.(!j) + !j;
  ...
  for _ = 1 to n - 1 do
    mt.(!i) <- (mt.(!i) lxor (mt.(!i - 1) lxor (mt.(!i - 1) lsr 30) * 1566083941)) - !i;
    mt.(!i) <- mt.(!i) land mask32;
    incr i;
    if !i >= n
    then (
...
(* CPython getrandbits for k <= 32: the top k bits of one word. Upstream's only consumer
   is _randbelow over small collections, so wider draws are refused rather than
   half-implemented. *)
