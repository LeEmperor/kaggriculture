...
| [`benchmark_baseline.md`](benchmark_baseline.md) | Initial scaffold | Reproducible timing of the first native PASS-only transition slice. |
...
| [`ocaml_migration_decisions.md`](ocaml_migration_decisions.md) | Adopted decision record | OCaml as the primary development language. Adopted 2026-08-21: the game plan's fixed native-language decision is revised, `fast_model/` is OCaml and past its Phase 4 trust gate, and steps 1–8 of the work plan have landed — including the OCaml DSL interpreter and the subprocess shim. Step 9 (`kag_sim play`/`evaluate`) is next. |
...
- `evaluation_protocol.md` for seed splits and champion-promotion rules.
