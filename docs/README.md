...
| [`dsl_seam_extension.md`](dsl_seam_extension.md) | Analysis with a measured gate | Why the DSL's game seam is the critical path to a competitive submission, the three levels of extension and what each costs across the seam files, what the Phase 6 league already priced (vocabulary extension: 0.319→0.757; multi-worker: 1.6% of bankroll), the frozen-vocabulary tradeoff, and the reusable probe protocol. Refines `policy_dsl.md`'s limits section. |
